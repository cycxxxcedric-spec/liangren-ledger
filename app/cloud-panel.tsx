...
export default function CloudPanel({data:ledger}: {data:Data}){
 ...
 useEffect(()=>{
  ...
  let valid=false;try{const l=JSON.parse(localStorage.getItem(LINK)||'null') as Link|null;valid=!!session&&l?.user===session.user.id;setInvite(valid?l!.snapshot.invite:null);setCloudInfo(valid?l!.snapshot:null);}catch{}
  setLinked(valid);setStatus(session?(valid?'等待同步':'已登录 · 请连接账本'):'本机保存 · 尚未登录云端');
  run();const timer=setInterval(run,15000);const dirty=()=>{if(!active.current){setStatus('本机已保存 · 尚未登录云端');return;}if(conflictRef.current){setStatus('本机有新修改，待处理同步冲突');return;}const raw=localStorage.getItem(LINK);let connected=false;try{connected=JSON.parse(raw||'null')?.user===active.current.user.id;}catch{}if(!connected){setStatus('本机已保存 · 已登录但尚未连接共享账本');return;}setStatus('本机有修改 · 等待同步');run();};window.addEventListener('ledger-local-update',dirty);window.addEventListener('online',run);const visible=()=>{if(document.visibilityState==='visible')run();};document.addEventListener('visibilitychange',visible);
  return()=>{cancelled=true;clearInterval(timer);window.removeEventListener('ledger-local-update',dirty);window.removeEventListener('online',run);document.removeEventListener('visibilitychange',visible);};
 },[session?.user.id]);
 ...
 {!session?<form onSubmit={e=>{e.preventDefault();void auth(false);}} className="cloud-form"><label>邮箱<input type="email" autoComplete="email" required value={email} onChange={e=>setEmail(e.target.value)}/></label><label>密码<input type="password" minLength={8} autoComplete="current-password" required value={password} onChange={e=>setPassword(e.target.value)}/></label><button className="button primary" disabled={busy}>登录</button><button type="button" className="button outline" disabled={busy||!email||password.length<8} onClick={()=>void auth(true)}>注册新账号</button><p>可使用管理员在 Supabase Authentication → Users 中创建的账号直接登录。自行注册需要邮件服务支持。</p></form>:<div className="cloud-form"><p>当前账号：{session.user.email}</p><section className="form-tip"><b>核对夫妻是否在同一本账</b><p>本机流水：{ledger.entries.length} 笔</p>{linked&&cloudInfo?<><p style={{overflowWrap:'anywhere'}}>账本编号：{cloudInfo.id}</p><p>上次确认的云端流水：{cloudInfo.data.entries.length} 笔 · 版本 {cloudInfo.revision}</p><p>两部手机的账本编号必须完全相同。编号不是邀请码；笔数相同也不代表内容一定相同。</p></>:<p>尚未连接共享账本。登录成功不等于已经加入伴侣账本。</p>}<p>若伴侣记录最多，先在她的手机确认云端保存成功，再在另一台手机加入她的账本；已有不同账本时，先保留双方数据再处理。</p></section>{!linked?<><p>首次连接前会保留一份本机恢复副本。下方“连接已有”和“加入”会将界面切换为云端账本，不会自动合并两本独立账本。</p><button className="button primary" disabled={busy} onClick={()=>void attach('create')}>把本机账本保存为新的云端账本</button><button className="button outline" disabled={busy} onClick={()=>void attach('read')}>连接这个账号已有的云端账本</button><label>伴侣的邀请码<input value={code} onChange={e=>setCode(e.target.value)} autoComplete="off"/></label><button className="button outline" disabled={busy||!code} onClick={()=>void attach('join')}>加入伴侣账本</button></>:<><p>已开启自动同步，每 15 秒检查另一部手机的修改。</p>{invite&&<label>邀请码（仅给你的伴侣）<input readOnly value={invite} onFocus={e=>e.target.select()}/></label>}<button className="button outline" disabled={busy} onClick={()=>void sync()}>立即同步</button></>}
