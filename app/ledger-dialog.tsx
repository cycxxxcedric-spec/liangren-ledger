...
export function DialogContent({className='',children,...props}:Primitive.Popup.Props){
 return <Primitive.Portal><Primitive.Backdrop className="ledger-backdrop"/><Primitive.Popup {...props} className={`ledger-surface ${className}`}><div className="ledger-surface-body">{children}</div><Primitive.Close className="ledger-surface-close" aria-label="关闭"><X size={22}/></Primitive.Close></Primitive.Popup></Primitive.Portal>;
}
